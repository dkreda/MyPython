...
class Section(object):
    ...
    def Parse(self):
        self.__ParamIndex={}
        for LineNo in xrange(self.__Start,self.__End):
...
class INIFile(BaseFile):
   ...
   def __contains__(self, item):
       Parameter=self.IniPathPattern.match(item) or re.match('\[(.+?)\]',item)
       if Parameter:
           Flag=Parameter.group(1) in self.__SecMap
           if len(Parameter.groups()) > 1:
               return Flag and Parameter.group(2) in self.__SecMap[Parameter.group(1)]
           else: return Flag
       else:
           raise Exception("Ilegal Ini Path \"%s\" should be \"[Section Name]Param\"" % item)
...
if __name__ == "__main__":
    ...
    TestClass['[test2]NewParam']="Nerw Parameter in this Section !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
